---
title: "Battery SOC Estimator Interactive Simulation"
...
The state-space equations for the true battery and the SOC estimator are:
...
V_\text{true}[k] = V_\text{OCV}(SOC[k]) + R_s \, I[k]
$$

- $k$ : Discrete time step index  
- $\Delta t$ : Time step (s)  
- $V_\text{OCV}$ is chosen as either the linear or nonlinear model depending on the simulation.  

---

### SOC Estimator (Luenberger Observer / Kalman Filter)
...
- $n_I(t)$, $n_V(t)$ : Measurement noise  
- $K$ : Observer gain  

**Euler's Method**

$$
\hat{SOC}[k] = \hat{SOC}[k-1] + \Delta t \left[ \frac{I[k-1] + n_I[k-1]}{Q} + K \left( V_\text{true}[k-1] + n_V[k-1] - \hat{V}[k-1] \right) \right]
$$

$$
\hat{V}[k] = V_\text{OCV}(\hat{SOC}[k]) + R_s \left(I[k] + n_I[k]\right)
$$

- $V_\text{OCV}$ can be linear or nonlinear depending on the chosen model.  
- Other variables as defined above.


## Results

When the true VOC/SOC model is used for the estimator, the SOC estimate converges smoothly to the true SOC despite noise. However, when the linear approximation is used, the estimator stays close but doesn't reach the true SOC outside of when the cubic and linear curves intersect. Adding noise does not alter this observation

Additionally with the true model, increasing K for the fixed gain observer decreases the amount of time required for the estimator to reach the true soc value, however setting it too high introduces additional noise. It's clear that the kalman filter apporaches the true value very fast, but tuning the process and observer noise is required to reduce fluctuation.

**Example results:**

- At 8.3 minutes under a **0.5 A discharge**, the SOC error is ~4.7%.  
- At **2.2 A charge/discharge**, the SOC error at 500 seconds is ~4.4%.  

After 500 seconds, the observer estimates the true SOC within 5% under nominal conditions. Smaller initial errors (starting closer to the real SOC) improve convergence in depleted batteries.
